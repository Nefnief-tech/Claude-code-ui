...
const mainRPC = BrowserView.defineRPC<MainRPC>({
	...
	handlers: {
		requests: {
			...
			gitStashList: ({ cwd }) => gitStashList(cwd),
			gitStashPush: ({ cwd, message }) => gitStashPush(cwd, message),
			gitStashPop: ({ cwd, index }) => gitStashPop(cwd, index),
			gitStashDrop: ({ cwd, index }) => gitStashDrop(cwd, index),
			ghPrList: ({ cwd }) => ghPrList(cwd),
			ghPrCreate: ({ cwd, title, body, base, draft }) => ghPrCreate(cwd, title, body, base, draft),
			gitGenerateCommitMessage: ({ cwd, apiKey, baseUri }) => generateCommitMessage(cwd, apiKey, baseUri),
